...
# -*- coding: utf-8 -*-
"""
Same as 2Constant_gridsearch-findoptimal.py script in Project1
...
data_dir = os.path.join(script_dir, 'results/final/reward')
heat = 51

all_eps = np.linspace(0, 1, heat, endpoint=True)
all_lr = np.linspace(0, 1, heat, endpoint=True)
all_unchosen = np.linspace(-1/8, 1/8, heat, endpoint=True)
print(all_unchosen)
#Take results of the gridsearch and find the highest reward that was obtained and the parameter values that did it.
...
for i in range(heat):
    advtitle = f'mr_var_eps{i}.csv'
    adv_dic = os.path.join(data_dir,advtitle)
    statitle = f'mr_sta_eps{i}.csv'
    sta_dic = os.path.join(data_dir,statitle)
    voltitle = f'mr_vol_eps{i}.csv'
    vol_dic = os.path.join(data_dir,voltitle)

    advdata = pd.read_csv(adv_dic)
    advdata = advdata.to_numpy()  
    advdata = advdata[:,3:]
    advdata = advdata[2:,:]

    stadata = pd.read_csv(sta_dic)
    stadata = stadata.to_numpy()  
    stadata = stadata[:,3:]
    stadata = stadata[2:,:]

    voldata = pd.read_csv(vol_dic)
    voldata = voldata.to_numpy()
    voldata = voldata[:,3:]
    voldata = voldata[2:,:]

    print(np.shape(advdata), np.shape(stadata), np.shape(voldata))
