...
# -*- coding: utf-8 -*-
"""
Same as 2Constant_gridsearch-plot.py script in Project1
...
matplotlib.rcParams['font.size'] = 65
script_dir = os.path.dirname(os.path.abspath(__file__))
folder_name = "results/reward"
save_dir = os.path.join(script_dir, folder_name)

eps0_adv_file = 'mr_var_eps0.csv'
eps0_adv_dir = os.path.join(save_dir, eps0_adv_file)
eps0_adv = pd.read_csv(eps0_adv_dir)
...
eps24_adv_file = 'mr_var_eps11.csv'
eps24_adv_dir = os.path.join(save_dir, eps24_adv_file)
eps24_adv = pd.read_csv(eps24_adv_dir)
eps24_adv=eps24_adv.to_numpy()
eps24_adv=eps24_adv[:,3:]
eps24_adv=eps24_adv[2:,:]
eps24_adv=eps24_adv[::-1]


eps72_adv_file = 'mr_var_eps34.csv'
eps72_adv_dir = os.path.join(save_dir, eps72_adv_file)
eps72_adv = pd.read_csv(eps72_adv_dir)
eps72_adv=eps72_adv.to_numpy()
eps72_adv=eps72_adv[:,3:]
eps72_adv=eps72_adv[2:,:]
eps72_adv=eps72_adv[::-1]



eps0_sta_file = 'mr_sta_eps0.csv'
eps0_sta_dir = os.path.join(save_dir, eps0_sta_file)
eps0_sta = pd.read_csv(eps0_sta_dir)
eps0_sta=eps0_sta.to_numpy()
eps0_sta=eps0_sta[:,3:]
eps0_sta=eps0_sta[2:,:]
eps0_sta=eps0_sta[::-1]

eps24_sta_file = 'mr_sta_eps11.csv'
eps24_sta_dir = os.path.join(save_dir, eps24_sta_file)
eps24_sta = pd.read_csv(eps24_sta_dir)
eps24_sta=eps24_sta.to_numpy()
eps24_sta=eps24_sta[:,3:]
eps24_sta=eps24_sta[2:,:]
eps24_sta=eps24_sta[::-1]

eps72_sta_file = 'mr_sta_eps34.csv'
eps72_sta_dir = os.path.join(save_dir, eps72_sta_file)
eps72_sta = pd.read_csv(eps72_sta_dir)
eps72_sta=eps72_sta.to_numpy()
eps72_sta=eps72_sta[:,3:]
eps72_sta=eps72_sta[2:,:]
eps72_sta=eps72_sta[::-1]



eps0_vol_file = 'mr_vol_eps0.csv'
eps0_vol_dir = os.path.join(save_dir, eps0_vol_file)
eps0_vol = pd.read_csv(eps0_vol_dir)
...
eps24_vol_file = 'mr_vol_eps11.csv'
eps24_vol_dir = os.path.join(save_dir, eps24_vol_file)
eps24_vol = pd.read_csv(eps24_vol_dir)
eps24_vol=eps24_vol.to_numpy()
eps24_vol=eps24_vol[:,3:]
eps24_vol=eps24_vol[2:,:]
eps24_vol=eps24_vol[::-1]

eps72_vol_file = 'mr_vol_eps34.csv'
eps72_vol_dir = os.path.join(save_dir, eps72_vol_file)
eps72_vol = pd.read_csv(eps72_vol_dir)
eps72_vol=eps72_vol.to_numpy()
eps72_vol=eps72_vol[:,3:]
eps72_vol=eps72_vol[2:,:]
...
cbar = plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
cbar.set_label('Average reward')
plt.title(r"Adversarial environment" "\n" "stochasticity ($\epsilon$) = 0.22")
plt.savefig(title, dpi=300, bbox_inches='tight')

title = os.path.join(save_dir, '1adveps68')
fig, ax = plt.subplots(figsize=(28, 28))
max_idx = np.unravel_index(np.argmax(eps72_adv), eps72_adv.shape)
im = ax.imshow(eps72_adv, vmin=0, vmax=1, cmap=cmappie)
ax.scatter(max_idx[1], max_idx[0], color='blue', marker='.', s=300)
ax.set_xlabel(r'Unchosen value-bias ($\lambda$)', fontsize=fontsize)
ax.set_ylabel(r'Learning rate ($\alpha$)', fontsize=fontsize)
ax.set_xticks(ticks=list(heat), labels=np.round(all_unchosen,3))  
ax.set_yticks(ticks=list(heat), labels=all_lr)  
...
im = ax.imshow(eps0_sta, vmin=0, vmax=1, cmap=cmappie)
ax.scatter(max_idx[1], max_idx[0], color='blue', marker='.', s=200)
ax.set_xlabel(r'Unchosen value-bias ($\lambda$)', fontsize=fontsize)
ax.set_ylabel(r'Learning rate ($\alpha$)', fontsize=fontsize)
ax.set_xticks(ticks=list(heat), labels=np.round(all_unchosen,3))  
ax.set_yticks(ticks=list(heat), labels=all_lr)  
...
title = os.path.join(save_dir, '1staeps22')
fig, ax = plt.subplots(figsize=(28, 28))
im = ax.imshow(eps24_sta, vmin=0, vmax=1, cmap=cmappie)
ax.set_xlabel(r'Unchosen value-bias ($\lambda$)', fontsize=fontsize)
ax.set_ylabel(r'Learning rate ($\alpha$)', fontsize=fontsize)
ax.set_xticks(ticks=list(heat), labels=np.round(all_unchosen,3))  
ax.set_yticks(ticks=list(heat), labels=all_lr)  
...
title = os.path.join(save_dir, '1staeps68')
fig, ax = plt.subplots(figsize=(28, 28))
im = ax.imshow(eps72_sta, vmin=0, vmax=1, cmap=cmappie)
ax.set_xlabel(r'Unchosen value-bias ($\lambda$)', fontsize=fontsize)
ax.set_ylabel(r'Learning rate ($\alpha$)', fontsize=fontsize)
ax.set_xticks(ticks=list(heat), labels=np.round(all_unchosen,3))  
ax.set_yticks(ticks=list(heat), labels=all_lr)  
...
im = ax.imshow(eps0_vol, vmin=0, vmax=1, cmap=cmappie)
ax.scatter(max_idx[1], max_idx[0], color='blue', marker='.', s=200)
ax.set_xlabel(r'Unchosen value-bias ($\lambda$)', fontsize=fontsize)
ax.set_ylabel(r'Learning rate ($\alpha$)', fontsize=fontsize)
ax.set_xticks(ticks=list(heat), labels=np.round(all_unchosen,3))  
...
title = os.path.join(save_dir, '1voleps22')
fig, ax = plt.subplots(figsize=(28, 28))
im = ax.imshow(eps24_vol, vmin=0, vmax=1, cmap=cmappie)
ax.set_xlabel(r'Unchosen value-bias ($\lambda$)', fontsize=fontsize)
ax.set_ylabel(r'Learning rate ($\alpha$)', fontsize=fontsize)
ax.set_xticks(ticks=list(heat), labels=np.round(all_unchosen,3))  
ax.set_yticks(ticks=list(heat), labels=all_lr)  
...
title = os.path.join(save_dir, '1voleps68')
fig, ax = plt.subplots(figsize=(28, 28))
im = ax.imshow(eps72_vol, vmin=0, vmax=1, cmap=cmappie)
ax.set_xlabel(r'Unchosen value-bias ($\lambda$)', fontsize=fontsize)
ax.set_ylabel(r'Learning rate ($\alpha$)', fontsize=fontsize)
ax.set_xticks(ticks=list(heat), labels=np.round(all_unchosen,3))  
ax.set_yticks(ticks=list(heat), labels=all_lr)  
