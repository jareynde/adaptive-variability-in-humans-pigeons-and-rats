...
#    model 7.2  eps, lr, uc, ucs, w, gamnr = params
        ...
        sub_gamnr = np.append(sub_gamnr, param_fits[3])
        sub_BIC = np.append(sub_BIC, BIC)
        sub_negLL = np.append(sub_negLL, negLL)
        sub_trials = np.append(sub_trials, trials)

        line_to_write = {"sub": count, "file": file_name, "eps": param_fits[0], "alpha": param_fits[1], "lambda": param_fits[2], "gamnr": param_fits[3], "BIC": BIC, "negLL": negLL, "trials": trials, "choices": K}
        with open("Ratall_M4_2.csv", 'a') as f_object:
            field_names = ["sub", "file", "eps", "alpha", "lambda", "gamnr", "BIC", "negLL", "trials", "choices"]
    ...
    line_to_write = {"sub": count, "eps": sub_av_eps, "alpha": sub_av_alpha, "lambda": sub_av_uc, "gamnr": sub_av_gamnr, "BIC": sub_av_BIC, "negLL": sub_av_negLL, "trials": sub_av_trials, "choices": K}
    with open("Ratav_M4_2.csv", 'a') as f_object:
