...
#    model 7.2  eps, lr, uc, ucs, w, gamnr = params
        ...
        sub_gamr = np.append(sub_gamr, param_fits[2])
        sub_BIC = np.append(sub_BIC, BIC)
        sub_negLL = np.append(sub_negLL, negLL)
        sub_trials = np.append(sub_trials, trials)

        line_to_write = {"sub": count, "file": file_name, "eps": param_fits[0], "alpha": param_fits[1], "gamr": param_fits[2], "BIC": BIC, "negLL": negLL, "trials": trials, "choices": K}
        with open("Ratall_M3_1.csv", 'a') as f_object:
            field_names = ["sub", "file", "eps", "alpha", "gamr", "BIC", "negLL", "trials", "choices"]
    ...
    line_to_write = {"sub": count, "eps": sub_av_eps, "alpha": sub_av_alpha, "gamr": sub_av_gamr, "BIC": sub_av_BIC, "negLL": sub_av_negLL, "trials": sub_av_trials, "choices": K}
    with open("Ratav_M3_1.csv", 'a') as f_object:
