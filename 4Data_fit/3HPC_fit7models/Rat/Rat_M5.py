...
#    model 7.2  eps, lr, uc, ucs, w, gamnr = params
        ...
        negLL = result.fun
        param_fits = result.x
        BIC = len(bounds5) * np.log(trials) + 2*negLL
        ...
        line_to_write = {"sub": count, "file": file_name, "eps": param_fits[0], "alpha": param_fits[1], "lambda": param_fits[2], "lambda_s" : param_fits[3], "w": param_fits[4], "BIC": BIC, "negLL": negLL, "trials": trials, "choices": K}
        with open("Ratall_M5.csv", 'a') as f_object:
            field_names = ["sub", "file", "eps", "alpha", "lambda", "lambda_s", "w", "BIC", "negLL", "trials", "choices"]
    ...
    line_to_write = {"sub": count, "eps": sub_av_eps, "alpha": sub_av_alpha, "lambda": sub_av_uc, "lambda_s": sub_av_ucs, "w": sub_av_w, "BIC": sub_av_BIC, "negLL": sub_av_negLL, "trials": sub_av_trials, "choices": K}
    with open("Ratav_M5.csv", 'a') as f_object:
