...
#    model 7.2  eps, lr, uc, ucs, w, gamnr = params
        ...
        sub_gamr = np.append(sub_gamr, param_fits[5])
        sub_BIC = np.append(sub_BIC, BIC)
        sub_negLL = np.append(sub_negLL, negLL)
        sub_trials = np.append(sub_trials, trials)

        line_to_write = {"sub": count, "file": file_name, "eps": param_fits[0], "alpha": param_fits[1], "lambda": param_fits[2], "lambda_s" : param_fits[3], "w": param_fits[4], "gamr": param_fits[5], "BIC": BIC, "negLL": negLL, "trials": trials, "choices": K}
        with open("Ratall_M7_1.csv", 'a') as f_object:
            field_names = ["sub", "file", "eps", "alpha", "lambda", "lambda_s", "w", "gamr", "BIC", "negLL", "trials", "choices"]
    ...
    sub_av_gamr = np.mean(sub_gamr)
    sub_av_BIC = np.mean(sub_BIC)
    sub_av_negLL = np.mean(sub_negLL)
    sub_av_trials = np.mean(sub_trials)

    line_to_write = {"sub": count, "eps": sub_av_eps, "alpha": sub_av_alpha, "lambda": sub_av_uc, "lambda_s": sub_av_ucs, "w": sub_av_w, "gamr": sub_av_gamr, "BIC": sub_av_BIC, "negLL": sub_av_negLL, "trials": sub_av_trials, "choices": K}
    with open("Ratav_M7_1.csv", 'a') as f_object:
