...
#    model 7.2  eps, lr, uc, ucs, w, gamnr = params
        ...
        negLL = result.fun
        param_fits = result.x
        BIC = len(bounds2) * np.log(trials) + 2*negLL
        ...
        line_to_write = {"sub": count, "file": file_name, "eps": param_fits[0], "alpha": param_fits[1], "lambda": param_fits[2], "BIC": BIC, "negLL": negLL, "trials": trials, "choices": K}
        with open("Ratall_M2.csv", 'a') as f_object:
            field_names = ["sub", "file", "eps", "alpha", "lambda", "BIC", "negLL", "trials", "choices"]
    ...
    line_to_write = {"sub": count, "eps": sub_av_eps, "alpha": sub_av_alpha, "lambda": sub_av_uc, "BIC": sub_av_BIC, "negLL": sub_av_negLL, "trials": sub_av_trials, "choices": K}
    with open("Ratav_M2.csv", 'a') as f_object:
        field_names = ["sub", "eps", "alpha", "lambda", "BIC", "negLL", "trials", "choices"]
        dictwriter_object = DictWriter(f_object, fieldnames = field_names)
        dictwriter_object.writerow(line_to_write)
        f_object.close()
