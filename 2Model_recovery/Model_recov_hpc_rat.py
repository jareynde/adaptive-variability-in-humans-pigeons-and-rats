...
#model3
def negll_RW_eps_uc(params, k, r):
...
BIC1 = np.zeros([amount,3])
BIC2 = np.zeros([amount,3])
BIC3 = np.zeros([amount,3])
BIC4 = np.zeros([amount,3])
...
### sim model 1: eps, alpha, lambda ###

    Q_alpha = random.uniform(0, 1)
    eps = random.uniform(0, 1)
    unchosen = random.uniform(-1/K, 1/K)
    ...
    #fit model 3: RW, fitting eps and alpha
    result = differential_evolution(negll_RW_eps_uc, bounds=bounds3, args=(k,r), strategy=strategy)
    negLL = result.fun
    param_fits = result.x

    BIC1[i,2] = len(bounds3) * np.log(T) + 2*negLL

    #fit model 4: random
    negLL = negll_RW_eps(k)
    BIC1[i,3] = 2*negLL
    
    line_to_write = {'sim' : i, 'eps-alpha-lambda' : BIC1[i,0], 'eps-alpha' : BIC1[i,1], 'eps-lambda' : BIC1[i,2], 'eps' : BIC1[i,3]}
    with open("RAT_eps_alpha_lambda.csv", 'a') as f_object:
        field_names = ["sim", "eps-alpha-lambda", "eps-alpha", "eps-lambda", "eps"]
...
### sim model 2: eps, alpha ###
    ...
    BIC2[i,2] = len(bounds3) * np.log(T) + 2*negLL
    ...
    line_to_write = {'sim' : i, 'eps-alpha-lambda' : BIC2[i,0], 'eps-alpha' : BIC2[i,1], 'eps-lambda' : BIC2[i,2], "eps" : BIC2[i,3]}
    with open("RAT_eps_alpha.csv", 'a') as f_object:
        field_names = ["sim", "eps-alpha-lambda", "eps-alpha", "eps-lambda", "eps"]
        dictwriter_object = DictWriter(f_object, fieldnames = field_names)
        dictwriter_object.writerow(line_to_write)
        f_object.close()
...
### sim model 3: eps, alpha ###
    ...
    BIC3[i,0] = len(bounds1) * np.log(T) + 2*negLL
    ...
    BIC3[i,1] = len(bounds2) * np.log(T) + 2*negLL
    ...
    BIC3[i,2] = len(bounds3) * np.log(T) + 2*negLL
    ...
    line_to_write = {'sim' : i, 'eps-alpha-lambda' : BIC3[i,0], 'eps-alpha' : BIC3[i,1], 'eps-lambda' : BIC3[i,2], 'eps' : BIC3[i,3]}
    with open("RAT_eps_lambda.csv", 'a') as f_object:
        field_names = ["sim", "eps-alpha-lambda", "eps-alpha", "eps-lambda", "eps"]
        dictwriter_object = DictWriter(f_object, fieldnames = field_names)
        dictwriter_object.writerow(line_to_write)
        f_object.close()
...
### sim model 4: eps ###
    ...
    BIC4[i,0] = len(bounds1) * np.log(T) + 2*negLL
    ...
    BIC4[i,1] = len(bounds2) * np.log(T) + 2*negLL
    ...
    BIC4[i,2] = len(bounds3) * np.log(T) + 2*negLL
    ...
    line_to_write = {'sim' : i, 'eps-alpha-lambda' : BIC4[i,0], 'eps-alpha' : BIC4[i,1], 'eps-lambda' : BIC4[i,2], "eps": BIC4[i,3]}
    with open("RAT_eps.csv", 'a') as f_object:
        field_names = ["sim", "eps-alpha-lambda", "eps-alpha", "eps-lambda", "eps"]
        dictwriter_object = DictWriter(f_object, fieldnames = field_names)
        dictwriter_object.writerow(line_to_write)
        f_object.close()
...
df_BIC3 = pd.DataFrame(BIC3)


# Save the DataFrame to an Excel file
df_BIC1.to_excel(os.path.join(save_dir, 'strictlambda/1rat_BIC_eps-alpha-lambda.xlsx'))
df_BIC2.to_excel(os.path.join(save_dir, 'strictlambda/2rat_BIC_eps-alpha.xlsx'))
df_BIC3.to_excel(os.path.join(save_dir, 'strictlambda/3rat_BIC_eps-lambda.xlsx'))
...
# Save the DataFrame to an Excel file
df_BIC1.to_excel(os.path.join(save_dir, 'strictlambda/1HP8_BIC_eps-alpha-lambda.xlsx'))
df_BIC2.to_excel(os.path.join(save_dir, 'strictlambda/2HP8_BIC_eps-alpha.xlsx'))
...
# Save the DataFrame to an Excel file
df_BIC1.to_excel(os.path.join(save_dir, 'strictlambda/1HP4_BIC_eps-alpha-lambda.xlsx'))
df_BIC2.to_excel(os.path.join(save_dir, 'strictlambda/2HP4_BIC_eps-alpha.xlsx'))
...
# Save the DataFrame to an Excel file
df_BIC1.to_excel(os.path.join(save_dir, 'strictlambda/1HP2_BIC_eps-alpha-lambda.xlsx'))
df_BIC2.to_excel(os.path.join(save_dir, 'strictlambda/2HP2_BIC_eps-alpha.xlsx'))
